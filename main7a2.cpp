...
/*The first template parameter of the index_appender_view class template is an STL container that can be indexed from 0, 
the second parameter is the type of data this container stores. 
A container of the same type as the first template parameter may be added to the view (add),
but the container may be changed externally.
Containers added in sequence will be indexed continuously using at/operator[].
It should also work in conjunction with vector<bool>.
Also, it is a requirement, to make the second template parameter optional.
*/
